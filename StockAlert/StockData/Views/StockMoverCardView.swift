...
struct StockMoverCardView: View {
  ...
  init(stockMover: StockMover) {
    ...
    percentageChange = percentFormatter.string(
        from: NSNumber(value: stockMover.change)
    )!
  }
...
struct StockMoverCardView_Previews: PreviewProvider {
  static var previews: some View {
    StockMoverCardView(
        stockMover: StockMover(
            change: 0.24046357615894032,
            description: "Heartbeam, Inc. - Common Stock",
            direction: "up",
            lastValue: 5.6193,
            symbol: "BEAT",
            totalVolume: 21227036)
    )
  }
}
